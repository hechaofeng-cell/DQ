#!/usr/bin/env python3
"""Pre-registered screen of four representative modern ImageNet models."""

from __future__ import annotations

import json
from collections import defaultdict

import run_four_model_gap_vs_random as experiment


MODELS = ("regnet_y_3_2gf", "efficientnet_v2_s", "maxvit_t", "vit_b_16")
SEEDS = (20260916, 20260917, 20260918)
OUTPUT = experiment.ROOT / "artifacts/modern_models_screen_20260917"


def main() -> None:
    experiment.MODEL_NAMES = MODELS
    experiment.SEEDS = SEEDS
    experiment.OUTPUT = OUTPUT
    OUTPUT.mkdir(parents=True, exist_ok=True)

    profiles = [experiment.model_profile(model_name) for model_name in MODELS]
    experiment.write_json(OUTPUT / "protocol.json", {
        "experiment_type": "pre-registered modern architecture screen",
        "models": list(MODELS),
        "conditions_in_run_order": list(experiment.CONDITIONS),
        "classes": [name for name, _ in experiment.CLASS_SPECS],
        "seeds": list(SEEDS),
        "epochs": 20,
        "samples_per_epoch": 3000,
        "batch_size": 64,
        "optimizer": "AdamW(lr=1e-4, weight_decay=1e-4)",
        "selection_metric": "validation macro-F1",
        "input_size": 224,
        "data_root": str(experiment.DATA_ROOT.resolve()),
        "gap_dog_manifest": str((experiment.DATA_ROOT / "data/train_dog596.csv").resolve()),
        "random_dog_manifest": str((experiment.DATA_ROOT / "data/train_dog_random596.csv").resolve()),
        "other_train_manifest": str((experiment.DATA_ROOT / "data/train_other_fixed.csv").resolve()),
        "eval_manifest": str((experiment.DATA_ROOT / "data/eval_fixed.csv").resolve()),
        "model_sources": {
            "regnet_y_3_2gf": "https://docs.pytorch.org/vision/stable/models/generated/torchvision.models.regnet_y_3_2gf.html",
            "efficientnet_v2_s": "https://docs.pytorch.org/vision/stable/models/generated/torchvision.models.efficientnet_v2_s.html",
            "maxvit_t": "https://docs.pytorch.org/vision/stable/models/generated/torchvision.models.maxvit_t.html",
            "vit_b_16": "https://docs.pytorch.org/vision/stable/models/generated/torchvision.models.vit_b_16.html",
        },
    })
    (OUTPUT / "research_log.md").write_text(
        "# Research Log\n\n"
        "- Decision: test whether coverage-driven dog100 gains extend to four pre-registered modern architectures.\n"
        "- Models fixed before results: RegNetY-3.2GF, EfficientNetV2-S, MaxVit-T, ViT-B/16.\n"
        "- Rationale: representative design-space CNN, efficient CNN, hybrid attention, and pure transformer families available in Torchvision 0.29.\n"
        "- Hypothesis: gap596 exceeds random596 on all seven core metric means.\n"
        "- Primary metrics: Macro-F1 and dog F1.\n"
        "- Strict positive-model rule: Accuracy, Macro-Precision/Recall/F1, and dog Precision/Recall/F1 mean deltas are all > 0.\n"
        "- Success: at least 2/4 models meet the strict positive-model rule.\n"
        "- Partial support: at least 2/4 improve both primary metrics but fail a secondary metric.\n"
        "- Stop criterion: one complete three-seed run per condition and model; no candidate replacement or post-result tuning.\n"
        "- Fixed factors: manifests, test set, transforms, optimizer, batch size, epochs, sampling budget, seeds.\n"
        "- Command: `.venv/bin/python -u scripts/run_modern_models_screen.py`\n"
        "- Boundary: one random100 list and a test set already used by earlier exploratory work.\n",
        encoding="utf-8",
    )

    gap_dogs = experiment.read_csv(experiment.DATA_ROOT / "data/train_dog596.csv")
    random_dogs = experiment.read_csv(experiment.DATA_ROOT / "data/train_dog_random596.csv")
    other_rows = experiment.read_csv(experiment.DATA_ROOT / "data/train_other_fixed.csv")
    eval_rows = experiment.read_csv(experiment.DATA_ROOT / "data/eval_fixed.csv")
    val_rows = [row for row in eval_rows if row["split"] == "val"]
    test_rows = [row for row in eval_rows if row["split"] == "test"]
    condition_rows = {
        "gap596": gap_dogs + other_rows,
        "random596": random_dogs + other_rows,
    }

    all_metrics = defaultdict(lambda: defaultdict(list))
    for model_name in MODELS:
        for condition in experiment.CONDITIONS:
            for seed in SEEDS:
                metrics = experiment.train_one(
                    model_name,
                    condition,
                    seed,
                    condition_rows[condition],
                    val_rows,
                    test_rows,
                )
                all_metrics[model_name][condition].append(metrics)

    summary, deltas = experiment.aggregate(all_metrics, profiles)
    experiment.make_report(summary, deltas, profiles)
    experiment.write_json(OUTPUT / "report.json", {
        "experiment_type": "pre-registered modern architecture screen",
        "coverage": {
            "random596": 0.8132791327913279,
            "gap596": 0.8417344173441734,
            "gap_minus_random": 0.0284552845528455,
        },
        "profiles": profiles,
        "summary": summary,
        "deltas": deltas,
        "conclusion": experiment.summarize_conclusion(deltas),
        "limitations": [
            "one random100 subset",
            "three training seeds",
            "test set reused from earlier exploratory work",
            "oracle target crops",
        ],
    })
    experiment.write_checksums()
    print(json.dumps({
        "output": str(OUTPUT.resolve()),
        "models": MODELS,
        "seeds": SEEDS,
    }, ensure_ascii=False, indent=2), flush=True)


if __name__ == "__main__":
    main()
