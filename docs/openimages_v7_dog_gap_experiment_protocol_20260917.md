...
版本：v1.0  
日期：2026-09-17  
状态：本地 VLM 特征标注及选样完成；四模型训练运行中，正式模型结果尚未产生
...
## 11. 2026-09-18 续跑与选样结果

前一晚的候选标注进程中断后，从已保存的结果续跑。候选池共有 3,000 张，其中一张首次响应违反嘴部与舌头可见性的依赖规则；同种子重试仍失败，更换推理种子后严格校验通过。接受的特征是本地 VLM 输出，不是人工修订；原始失败的完整响应被旧流水线同名文件覆盖，已知失败字段及复试过程记录在 `artifacts/openimages_v7_dog_candidate_features_v1/retry_audit.md`。最终基线 1,500/1,500、候选 3,000/3,000 合法，不能误称全部首次成功。
...
|---|---:|---:|
| base | 1,500 | 86.86% |
| random300 | 1,800 | 87.89% |
| gap300 | 1,800 | 91.73% |

`gap300 - random300` 为 +3.85 个百分点；上述不是分类准确率。四模型固定测试集结果仍待完整训练和原始结果审计。
